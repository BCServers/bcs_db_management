# bcs_db_management
A set of scripts for managing the [website](https://github.com/BCServers/bcs_website)
databases (`server_db.json` and `user_db.json`) and generating the AWG
(`wg0.conf` and `clientsTable`) and Xray (`server.json` and `clientsTable`)
server configuration files from them.
